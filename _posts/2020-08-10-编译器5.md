...
如果自动机是无二义的，即每一个状态对于一个输入仅存在唯一的动作选择，那么这种自动机称为“确定有限状态自动机”，而如果自动机是二义的，那么这种自动机称为“不确定有限状态自动机”。自动机的属性与正则表达式的形式密切相关：  
如：下述两个正则:
